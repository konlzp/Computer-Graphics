...
function drawThePrimitive(gl, primitive) {
	...
	if (primitive.name == "cube") {
		...
		viewMat = SglMat4.lookAt([0,-1,3], [0,1,0], [0,1,1]); 
	}
		...
		viewMat = SglMat4.lookAt([0,-1,4], [0,0,0], [0,1,1]); 
	}	
	else if (primitive.name == "torus") {
		viewMat = SglMat4.lookAt([0,-1,4], [0,0,0], [0,1,1]); 
	}
	else if (primitive.name == "bspline-patch") {
		viewMat = SglMat4.lookAt([3,6,3], [-1,0,-1], [0,1,0]); 
	}
	else if (primitive.name == "loopSub") {
		viewMat = SglMat4.lookAt([0,-1,4], [0,0,0], [0,1,1]);
	}
	...
	//modelviewprojMat = SglMat4.mul(modelviewprojMat, SglMat4.scaling([0.05,0.3,0.3]));
