...
NVMCClient.drawWheel = function (gl) {
	...
	if(leftFlag || rightFlag) {
		if(steelCount < 30 && steelAngle < 30) {
			steelAngle += 1;
			steelCount += 1;
		}
		else if(steelCount >= 30){
			steelAngle -= 1;
		}
		if(steelAngle == 0) {
			steelCount = 0;
	...
	if(leftFlag > 0) {
		var M_3_rot1 = SglMat4.rotationAngleAxis(sglDegToRad(steelAngle), [1, 0, 0]);
		stack.multiply(M_3_rot1);
	}
	if(rightFlag > 0) {
		var M_3_rot2 = SglMat4.rotationAngleAxis(sglDegToRad(-steelAngle), [1, 0, 0]);
		stack.multiply(M_3_rot2);
	}
	if(game.playerAccelerate || game.playerBrake) {
		currentAngle += 3 * forwardFlag;
		var M_3_rot3 = SglMat4.rotationAngleAxis(sglDegToRad(currentAngle), [0, 1, 0]);
		stack.multiply(M_3_rot3);
	}
...
NVMCClient.drawCar = function (gl) {
	...
	var M_7 = SglMat4.translation([1.1, 0.3, 1.4]);
	stack.multiply(M_7);
	this.drawWheel(gl);
	...
	var M_5 = SglMat4.translation([-1.1, 0.3, 1.4]);
	stack.multiply(M_5);
	this.drawWheel(gl);
	...
	var M_2_tra_0 = SglMat4.translation([0, 0.3, 2]);
	stack.multiply(M_2_tra_0);

	var M_2_sca = SglMat4.scaling([1, 0.5, 4]);
	...
	//Changed Code
	stack.multiply(SglMat4.translation([0, -1, -1]));
	stack.multiply(SglMat4.rotationAngleAxis(sglDegToRad(180), [0, 1, 0]));
	stack.multiply([
		2, 0, 0, 0,
		0, 2, 0, 0,
		0, 0, 1, 1,
...
NVMCClient.drawScene = function (gl) {
	...
	var invV = SglMat4.lookAt([0.1, 1, 0], [0, 0, 0], [1, 0, 0]);
	stack.multiply(invV);
	stack.push();//line 242
